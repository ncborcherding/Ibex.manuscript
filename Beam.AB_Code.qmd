...
epitopeMatrix <- as.matrix(SeuratMerge@assays$BEAM$counts[1:5,])

lapply(seq_len(ncol(epitopeMatrix)), function(x) {
  delta <- sort(epitopeMatrix[,x])[5] - sort(epitopeMatrix[,x])[4]
  if(delta > sort(epitopeMatrix[,x])[4]) {
...
epitope.assignments$Epitope <- str_split(epitope.assignments$Epitope, "-", simplify = TRUE)[,2]
