...
colorblind_vector <- hcl.colors(n=7, palette = "viridis", fixup = TRUE)
```

## Loading and Filtering Single-Cell Data

```{r}
SC.Files <- list.files("./data/BEAM.AB/Dugan2021")
lapply(SC.Files, function(x) {
      tmp <- Read10X(data.dir = paste0("./data/BEAM.AB/Dugan2021/", x))
      ...
      contigs <- read.csv(paste0("./data/BEAM.AB/Dugan2021/", x, "/filtered_contig_annotations.csv.gz"))
...
plot1 <- DimPlot(SeuratMerge)
plot2 <- DimPlot(SeuratMerge, group.by = "orig.ident")
plot3 <- DimPlot(SeuratMerge, group.by = "cloneSize") + 
  scale_color_viridis(discrete = TRUE)
plot4 <-  DimPlot(SeuratMerge, group.by = "predicted.celltype.l2") + 
  scale_color_viridis(discrete = TRUE, option = "H")

saveRDS(SeuratMerge, "./data/processed/Dugan2021_BEAM.ab_Seurat.rds")
```
...
```{r}
FeaturePlot(SeuratMerge, features = rownames(SeuratMerge@assays$BEAM)[6:10])
