...
plot1 <- ggplot(tuned.results[tuned.results$variable == "hidden_dim1",], 
    ...
    geom_smooth(aes(color = model), lwd = 0.5, alpha = 0.7) + 
...
plot3 <- ggplot(tuned.results[tuned.results$variable == "hidden_dim2",], 
       aes(x=as.numeric(value), y = -log10(as.numeric(score)), group = model)) + 
    geom_smooth(aes(color = model), lwd = 0.5) + 
  facet_grid(optimizer~approach) + 
    ylab("-log10(mean_squared_error)") + 
    xlab("Layer 2 Size") + 
    scale_color_viridis(discrete = TRUE) + 
...
ggplot(tuned.results[tuned.results$method != "OHE" & tuned.results$variable == "hidden_dim1" & tuned.results$optimizer != "sgd",], 
