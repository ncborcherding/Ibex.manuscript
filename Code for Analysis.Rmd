...
for (j in seq_along(heavy)) {
        ...
        MDS.plot.H <- DimPlot(tmp, reduction = "mds", group.by = "Heavy.V") + 
         ...
         Dim.plot.H + Dim.plot.Ident + MDS.plot.H + MDS.plot.Ident + MDS.plot.Heavy + MDS.plot.HeavyL + plot_layout(ncol = 2)
    ggsave(paste0("./output/", heavy[j], "_", paste0(c("P1.1", "P2.1", "HD1", "HD4"), collapse = "."), ".png"), height = 12, width = 8, dpi = 600)
}
...
for (k in seq_along(light)) {
        ...
        MDS.plot.H <- DimPlot(tmp, reduction = "mds", group.by = "Light.V") + 
         ...
         Dim.plot.H + Dim.plot.Ident + MDS.plot.H + MDS.plot.Ident + MDS.plot.Light + MDS.plot.LightL + plot_layout(ncol = 2)
    ggsave(paste0("./output/", light[k], "_", paste0(c("P1.1", "P2.1", "HD1", "HD4"), collapse = "."), ".png"), height = 12, width = 8)
}
```
...
light.plot.Cl <- DimPlot(seurat.merge[,cells], 
                         reduction = "Ibex.L.KF", 
                         group.by = "Light.group", 
                         order = ) + 
                      scale_color_viridis(discrete = TRUE, option = "H", na.value = "grey") + 
                      theme_void() + 
                      theme(plot.title = element_blank()) 
ggsave("./output/Ibex_light_cluster.pdf", height = 3.5, width = 6)

heavy.plot.Cl <-DimPlot(seurat.merge[,cells],
                        reduction = "Ibex.H.KF",
                        group.by = "Heavy.group") + 
                      scale_color_viridis(discrete = TRUE, option = "H", na.value = "grey") + 
                      theme_void() + 
                      theme(plot.title = element_blank()) 
ggsave("./output/Ibex_heavy_cluster.pdf", height = 3.5, width = 5)
```
...
```{r}
conga <- CoNGAfy(seurat.merge[cells,], 
                 assay = c("RNA", "ADT"))
...
```{r}
cells <- which(seurat.merge[[]]$orig.ident %in% c("P3.2", "P3.1", "P4.1", "P4.2"))

meta <- seurat.merge[[]][cells,]
meta <- data.frame(meta, seurat.merge@reductions$Ibex.H.KF@cell.embeddings[cells,])
meta <- meta[,c(1,18:19,27:57)]
x <- calculateTSNE(t(meta[,5:34]))
x <- as.data.frame(x)
