...
unwanted_genes <- grep(pattern = unwanted_genes, x = features, value = TRUE)
unwanted_genes <- c(unwanted_genes , "JCHAIN")
features <- features[features %!in% unwanted_genes]
...
plot.df <- data.frame(SeuratObj[[]], 
                      SeuratObj@reductions$Ibex.H.KF@cell.embeddings[,c(1:2)], 
                      SeuratObj@reductions$Ibex.L.KF@cell.embeddings[,c(1:2)])

ggplot(plot.df, aes(x = IbexHKF_1, y = IbexHKF_2)) + 
  geom_point(aes(size = n, color = Heavy.V))
...
```{r}
BCR.contigs <- read.csv("./data/GSM5073091_PBMC_P1.1_MIS-C_Severe_BCR_filtered_contig_annotations.csv.gz")
BCR.contigs <- BCR.contigs[BCR.contigs$productive == "True",]
BCR.contigs <- unique(BCR.contigs[,c("barcode", "raw_clonotype_id")])
BCR.contigs$barcode <- paste0("MIS.sample_", BCR.contigs$barcode)

barcodes <- BCR.contigs$barcode

BCR.contigs <- data.frame(clonotype = BCR.contigs[,2])
rownames(BCR.contigs) <- barcodes

SeuratObj <- AddMetaData(SeuratObj, BCR.contigs)

light.plot.Cl <- DimPlot(SeuratObj, reduction = "Ibex.L.KF", group.by = "clonotype") + 
...
heavy.plot.Cl <-DimPlot(SeuratObj, reduction = "Ibex.H.KF", group.by = "clonotype") + 
                      scale_color_viridis(discrete = TRUE, option = "H") + 
                      theme_void() + 
                      guides(fill = "none") + 
                      theme(plot.title = element_blank()) +
...
heavy.plot.Cl
ggsave("./output/Ibex_heavy_cluster.pdf", height = 3.5, width = 3.5)
light.plot.Cl
ggsave("./output/Ibex_light_cluster.pdf", height = 3.5, width = 3.5)
