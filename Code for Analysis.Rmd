...
## Subsetting and visualizing latent dimensions
...
DimPlot(seurat.merge, reduction = "Ibex.L.KF", group.by = "Light.V", dims = c(3,4)) + 
                  scale_color_viridis(discrete = TRUE) + 
                  theme_void() + 
                  guides(fill = "none") + 
                  theme(plot.title = element_blank()) +
...
DimPlot(seurat.merge, reduction = "Ibex.L.AF", group.by = "Light.V", dims = c(1,4)) + 
                  scale_color_viridis(discrete = TRUE) + 
                  theme_void() + 
                  guides(fill = "none") + 
                  theme(plot.title = element_blank()) +
...
DimPlot(seurat.merge, reduction = "Ibex.L.OHE", group.by = "Light.V", dims = c(2,3)) + 
                  scale_color_viridis(discrete = TRUE) + 
                  theme_void() + 
                  guides(fill = "none") + 
                  theme(plot.title = element_blank()) +
...
for (i in 1:2) {
  ...
  Ben.cluster + bar.plot + Ibex.cluster + JI.plot / DI.plot + 
