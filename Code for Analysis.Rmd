...
## Loading BCR sequences that the autoencoder has never seen before. The R folder contains stripped down version of the Ibex encoders to allow for the comparisons without the need for single-cell data.
...
```{r}
BCR.contigs <- repLoad("./data/GSM5073091_PBMC_P1.1_MIS-C_Severe_BCR_filtered_contig_annotations.csv.gz")

<<<<<<< Updated upstream
distBCR <- seqDist(BCR.contigs$data  %>% top(500))

BCR.contigs$data <- seqCluster(BCR.contigs$data  %>% top(500), distBCR, .perc_similarity = 0.9)

shm_data <- BCR.contigs$data %>%
=======
distBCR <- seqDist(BCR.contigs$data %>% top(500))

align_dt <- BCR.contigs$data %>%
  seqCluster(seqDist(BCR.contigs$data, .col = 'CDR3.nt', .group_by_seqLength = TRUE), 
             .perc_similarity = 0.6) %>%
  repGermline(.threads = 1) %>%
  repAlignLineage(.min_lineage_sequences = 6, .align_threads = 2, .nofail = TRUE)

shm <- BCR.contigs$data %>%
>>>>>>> Stashed changes
