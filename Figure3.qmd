...
# Set the number of nearest neighbors and sigma for the Gaussian kernel
K <- 20
sigma <- 0.5

# Function to compute a sparse affinity matrix using kNN distances
compute_affinity_sparse <- function(data, K = 20, sigma = 0.5) {
  # data: cells x features (each row is a cell)
  # Use RANN to compute K+1 neighbors (first neighbor is the cell itself)
  nn_out <- nn2(data, k = K + 1)
  
  # Remove the first column (self-distance = 0)
  nn_idx <- nn_out$nn.idx[, -1]
  nn_dists <- nn_out$nn.dists[, -1]
  
  # Compute affinities using a Gaussian kernel
  affinities <- exp(- (nn_dists^2) / (2 * sigma^2))
  
  # Build a sparse matrix: for each cell, assign affinity values only for its K neighbors
  n <- nrow(data)
  rows <- rep(seq_len(n), each = K)
  cols <- as.vector(t(nn_idx))
  vals <- as.vector(t(affinities))
  
  # Create the sparse affinity matrix
  W <- sparseMatrix(i = rows, j = cols, x = vals, dims = c(n, n))
  # Symmetrize the matrix by averaging with its transpose
  W <- (W + t(W)) / 2
  return(W)
}

# Ensure that your matrices have cells as rows.
# For example, if corrected.pca, Heavy, and Light have cells in rows, you can use them directly.
# (Double-check with: dim(corrected.pca), dim(Heavy), dim(Light))

# Compute sparse affinity matrices for each modality using kNN
W1 <- compute_affinity_sparse(corrected.pca, K = K, sigma = sigma)
W2 <- compute_affinity_sparse(Heavy, K = K, sigma = sigma)
W3 <- compute_affinity_sparse(Light, K = K, sigma = sigma)

# SNF's SNF() function expects dense matrices. Depending on your memory constraints,
# you might convert the sparse matrices to dense. (This dense conversion is the next bottleneck.)
W1_dense <- as.matrix(W1)
W2_dense <- as.matrix(W2)
W3_dense <- as.matrix(W3)

# Fuse the affinity matrices
fused_W <- SNF(list(W1_dense, W2_dense, W3_dense), K = K)

# Apply a diffusion or spectral embedding to the fused network
dm <- diffuse(fused_W, t = 1)  # 'dm' typically returns eigenvectors

integrated_matrix <- dm  # Use the eigenvectors as the integrated matrix

# Run PHATE on the integrated matrix
phate_output <- phateR::phate(integrated_matrix)

# (Now add the embeddings back to your Seurat object as before)
SeuratMerge[["snf.integrated"]] <- CreateDimReducObject(
...
SeuratMerge[["snf.phate"]] <- CreateDimReducObject(
