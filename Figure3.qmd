...
block_pls_res <- block.pls(
  X = data_list,
  indY = 1,                      
  ncomp = 10,
  design = matrix(1, ncol = 3, nrow = 3),  # Equal connection 
  scheme = "horst",          
  max.iter = 500)

# combining the variates from all blocks:
combined_variates <- (block_pls_res$variates$RNA +
                      block_pls_res$variates$Light +
                      block_pls_res$variates$Heavy) / 3

phate_output <- phateR::phate(combined_variates)

SeuratMerge[["bpls.integrated"]] <- CreateDimReducObject(
  embeddings = combined_variates,
  stdev = rep(0, ncol(combined_variates)),
  assay = "RNA",
  key = "mm",
  jackstraw = NULL,
  misc = list()
