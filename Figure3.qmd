...
# Compute distance matrices and then affinity matrices for each modality
dist_rna   <- as.matrix(dist(corrected.pca))
dist_bcr1  <- as.matrix(dist(Heavy))
dist_bcr2  <- as.matrix(dist(Light))

# Convert distances to affinities; adjust K and sigma as needed
W1 <- affinityMatrix(dist_rna,  K = 20, sigma = 0.5)
W2 <- affinityMatrix(dist_bcr1, K = 20, sigma = 0.5)
W3 <- affinityMatrix(dist_bcr2, K = 20, sigma = 0.5)

# Fuse the affinity matrices into one network
fused_W <- SNF(list(W1, W2, W3), K = 20)

# Apply a diffusion or spectral embedding to the fused network
# Here, for example, using a diffusion map approach:
dm <- diffuse(fused_W, t = 1)
integrated_matrix <- dm  # dm typically gives you the eigenvectors
phate_output <- phateR::phate(integrated_matrix)

SeuratMerge[["snf.integrated"]] <- 
  CreateDimReducObject(embeddings = integrated_matrix,
                       stdev = rep(0, ncol(integrated_matrix)),
                       assay = "RNA",
                       key = "mm",
