...
```{r}
#' @title Comprehensive Binary Classification Summary for Caret
#' @description This function calculates a suite of binary classification metrics.
#' It is designed to be used as a `summaryFunction` within the `caret` package.
#' @param data A data frame with columns `obs` (observed labels) and `pred` (predicted labels).
#' It may also contain columns for class probabilities (e.g., `Yes`, `No`).
#' @param lev A character vector of the class levels. The second level is assumed to be the "positive" class.
#' @param model The `caret` model object (optional, not used here).
#' @return A named numeric vector containing:
#' \itemize{
#'   \item `Accuracy`: Overall accuracy.
#'   \item `Kappa`: Cohen's Kappa.
#'   \item `F1`: The F1-score (harmonic mean of precision and recall).
#'   \item `Sensitivity`: The sensitivity (or recall).
#'   \item `Specificity`: The specificity.
...
#' @title Jaccard Index for Cluster Partitions
#' @description Calculates the Jaccard index between two cluster partitions.
#' The index measures the similarity of co-clustering. It is defined as the
#' number of pairs of points clustered together in both partitions divided by
#' the number of pairs clustered together in at least one partition.
#' @param c1 An integer vector representing the first cluster partition.
#' @param c2 An integer vector representing the second cluster partition. Must be of the same length as c1.
#' @return The Jaccard index, a numeric value between 0 and 1.
#' Returns NA if no pairs are co-clustered in either partition.
...
# It requires the 'bluster' package for efficient kNN graph construction.
...
for (reduction_name in names(IGH.reduction.list)) {
  ...
  snn_graph <- buildSNNGraph(embedding_matrix, k = k_neighbors, type = "jaccard")
  
  # Run Leiden clustering on the graph
  partition <- leiden(snn_graph, resolution_parameter = 0.5) # Using a moderate resolution
  
  # Store the resulting cluster memberships
  cluster.assignments[[reduction_name]] <- partition
}
...
```{r}
# Register a parallel backend to speed up model training
# Using half of the available cores
...
for (reduction_name in names(all_results)) {
  for (m in names(all_results[[reduction_name]])) {
    ...
    if (!is.null(fit_obj)) {
      # The results from the cross-validation are in fit_obj$results
      # We want the metrics for the final, best model.
      # Since we used a custom CV strategy, we look at the single row of metrics.
      res_sub <- fit_obj$results[1, ]
      ...
      perf_list[[paste0(reduction_name, "_", m)]] <- data.frame(
        ...
        Kappa = res_sub$Kappa,
        F1 = res_sub$F1,
        Sensitivity = res_sub$Sensitivity,
        Specificity = res_sub$Specificity,
        PPV = res_sub$PPV,
        NPV = res_sub$NPV
      )
    }
...
# Sort the embedding methods by their median F1 score for a clearer plot
final_perf$Reduction_f <- reorder(final_perf$Reduction_clean, final_perf$F1, median)

# Create the dot plot
ggplot(final_perf, aes(x = F1, y = Reduction_f, color = Model)) +
  geom_point(alpha = 0.8, size = 4) +
  scale_color_brewer(palette = "Set1", name = "ML Model") +
  labs(
...
In this analysis, we rigorously benchmarked multiple deep learning and classical embedding techniques for representing BCR heavy chain sequences. We evaluated these embeddings on their ability to preserve intrinsic data structure (via clustering) and their power in a supervised prediction task (antigen specificity).

**Key Findings:**

1.  **Clustering (Figure 4C):** The clustering analysis revealed that embeddings generated with similar methods (e.g., all VAE-based models) tended to produce highly similar clusterings, as shown by the high ARI scores among them. The multimodal embeddings, which combined BCR information with RNA-seq PCA (`+RNA`), often formed a distinct group, suggesting they capture a different, yet consistent, structural representation of the data.

2.  **Predictive Performance (Figure 4D):** In the task of predicting spike protein specificity, the multimodal embeddings (`+RNA`) were the clear top performers. For almost every embedding type, adding the RNA information dramatically boosted the F1-score. The `ranger` (Random Forest) model was the most consistent and highest-performing model architecture across nearly all embeddings, highlighting its robustness. The `Benisse` and `Ibex.H.VAE.OHE+RNA` embeddings, when paired with the `ranger` model, achieved the highest F1-scores, demonstrating excellent predictive power.

3.  **ROC Analysis (Figure 4E):** The ROC analysis of the best model (`ranger`) confirmed the superiority of the multimodal embeddings. The curves for the `+RNA` methods consistently showed a larger Area Under the Curve (AUC), indicating better discrimination between spike-specific and non-specific cells across all classification thresholds.

**Overall Conclusion:**

For the task of antigen specificity prediction, a multimodal approach that combines BCR sequence embeddings with transcriptional data offers substantially better performance than using BCR information alone. While VAE- and CNN-based autoencoders produce useful representations, their predictive power is significantly enhanced when concatenated with PCA components from the single-cell gene expression data. The combination of a VAE with one-hot encoding, augmented with RNA data, and fed into a Random Forest classifier, provided the best results in this benchmark.
