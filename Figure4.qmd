...
```{r}
set.seed(123)
# Step 1: Create training/test split at the clonotype level
unique_clonos <- unique(IGH.clonotypes)
test_clonos <- sample(unique_clonos, size = floor(0.2 * length(unique_clonos)))
test_idx <- which(IGH.clonotypes %in% test_clonos)
train_idx <- setdiff(seq_along(IGH.clonotypes), test_idx)

# Step 2: Prepare training folds (CV) at clonotype level
train_clonos <- IGH.clonotypes[train_idx]
train_clono_ids <- unique(train_clonos)
clono_folds <- createFolds(train_clono_ids, k = 5, returnTrain = FALSE)

train_index_list <- list()
test_index_list  <- list()

for (fold_i in seq_along(clono_folds)) {
  fold_clonos <- train_clono_ids[clono_folds[[fold_i]]]
  test_fold_idx <- train_idx[which(train_clonos %in% fold_clonos)]
  train_fold_idx <- setdiff(train_idx, test_fold_idx)
  
  train_index_list[[fold_i]] <- train_fold_idx
  test_index_list[[fold_i]]  <- test_fold_idx
}
...
# Step 3: Define trainControl using CV folds
fitControl <- trainControl(
  method = "cv",
  number = 5,
  index = train_index_list,
...
ml_methods <- c("rf", "xgbLinear", "lda", "knn")
all_results <- list()

# Step 4: Loop over each dimensionality reduction
for (reduction_name in names(IGH.reduction.list)) {
  ...
  embedding <- embedding[, apply(embedding, 2, sd, na.rm = TRUE) != 0]
  ...
  # Step 5: Define recipe (scaling and filtering)
